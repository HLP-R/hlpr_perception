...
parseArguments(int argc, char **argv, parsedArguments &pA, ros::NodeHandle &nh)
{
 ...
 // {
      if (nh.getParam("nonplanar_feature_extraction/v", val))
      {
          pA.hue_val = val;
      }
      if (nh.getParam("nonplanar_feature_extraction/t", val))
      {
          pA.hue_thresh = val;
      }
      if (nh.getParam("nonplanar_feature_extraction/z", val))
      {
          pA.z_thresh = val;
      }
      if (nh.getParam("nonplanar_feature_extraction/e", val))
      {
          pA.euc_thresh = val;
      }
      if (nh.getParam("nonplanar_feature_extraction/p", bVal))
      {
          pA.pre_proc = bVal;
      }
      if (nh.getParam("nonplanar_feature_extraction/c", iVal))
      {
          pA.seg_color_ind = iVal;
      }
      if (nh.getParam("nonplanar_feature_extraction/m", bVal))
      {
          pA.merge_clusters = bVal;
      }
      if (nh.getParam("nonplanar_feature_extraction/dt", val))
      {
          pA.ecc_dist_thresh = val;
      }
      if (nh.getParam("nonplanar_feature_extraction/ct", val))
      {
          pA.ecc_color_thresh = val;
      }
      if (nh.getParam("nonplanar_feature_extraction/src", iVal))
      {
    	  pA.pc_source = iVal;
      }
      if (nh.getParam("nonplanar_feature_extraction/out", iVal))
      {
          pA.output_type = iVal;
      }
      if (nh.getParam("nonplanar_feature_extraction/comm", iVal))
      {
    	  pA.comm_medium = iVal;
      }
      if (nh.getParam("nonplanar_feature_extraction/rt", sVal))
      {
          sprintf(pA.ros_topic, "%s", sVal.c_str());
      }
      if (nh.getParam("nonplanar_feature_extraction/b", iVal))
      {
          pA.displayAllBb = iVal;
      }
      if (nh.getParam("nonplanar_feature_extraction/sh", iVal))
      {
          pA.saturation_hack = iVal;
      }
      if (nh.getParam("nonplanar_feature_extraction/st", val))
      {
          pA.saturation_hack_value = val;
      }
      if (nh.getParam("nonplanar_feature_extraction/sv", val))
      {
          pA.saturation_mapped_value = val;
      }
      if (nh.getParam("nonplanar_feature_extraction/ri", iVal))
      {
          pA.robot_id = iVal;
      }
      if (nh.getParam("nonplanar_feature_extraction/pr", iVal))
      {
    	  pA.freenectProcessor = iVal;
      }
      if (nh.getParam("nonplanar_feature_extraction/fn", iVal))
      {
    	  pA.filterNoise = iVal;
      }
      if (nh.getParam("nonplanar_feature_extraction/nv", bVal))
      {
    	  pA.viz = !bVal;
      }
      if (nh.getParam("nonplanar_feature_extraction/view", iVal))
      {
    	  pA.justViewPointCloud = iVal; 
      }
      if (nh.getParam("nonplanar_feature_extraction/h", bVal))
      {
