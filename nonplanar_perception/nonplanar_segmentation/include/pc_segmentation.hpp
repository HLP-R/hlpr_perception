...
// Defined by Priyanka - Testing out object detection for non-planar segmentation
...
//#endif
 ...
 // void pcl::compute3DCentroid(pcl::PointCloud<PointT> &cluster, Eigen::Vector4f centroid);
  ...
  void
  displayPlane(pcl::PointCloud<PointT>::Ptr &contour, int plane_num);

  void
  displayPlanes(std::vector<pcl::PointCloud<PointT>::Ptr> &contours);
  ...
  void 
  computePlaneModels(std::vector<pcl::PlanarRegion<PointT>,     Eigen::aligned_allocator<pcl::PlanarRegion<PointT> > > &regions,
             std::vector<Eigen::Vector4f> &plane_coefficients,                       std::vector<pcl::PointCloud<PointT>::Ptr> &contours);

  void 
  computePlaneModels(std::vector<pcl::PlanarRegion<PointT>,        Eigen::aligned_allocator<pcl::PlanarRegion<PointT> > > &regions,               std::vector<Eigen::Vector4f> &plane_coefficients);
